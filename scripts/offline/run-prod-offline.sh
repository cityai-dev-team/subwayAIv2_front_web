...
if [ "$ENVIRONMENT" != "production" ]; then
    echo -e "${YELLOW}⚠️  경고: ENVIRONMENT가 'production'이 아닙니다. (현재: ${ENVIRONMENT:-not set})${NC}"
...
build_image() {
    echo -e "${YELLOW}🔨 운영 환경 Docker 이미지 빌드 중 (오프라인 모드)...${NC}"
    echo -e "${BLUE}[INFO] 빌드 시 --prefer-offline을 사용하여 캐시 우선, 없으면 네트워크에서 다운로드합니다.${NC}"
    ...
    if [ -n "$VITE_API_BASE" ]; then
        BUILD_ARGS="$BUILD_ARGS --build-arg VITE_API_BASE=$VITE_API_BASE"
    fi
    if [ -n "$VITE_API_PREFIX" ]; then
        BUILD_ARGS="$BUILD_ARGS --build-arg VITE_API_PREFIX=$VITE_API_PREFIX"
    fi
    if [ -n "$VITE_APP_TITLE" ]; then
        BUILD_ARGS="$BUILD_ARGS --build-arg VITE_APP_TITLE=\"$VITE_APP_TITLE\""
    fi
    if [ -n "$VITE_APP_VERSION" ]; then
        BUILD_ARGS="$BUILD_ARGS --build-arg VITE_APP_VERSION=$VITE_APP_VERSION"
    fi
    
    IMAGE_NAME_FULL="${IMAGE_NAME:-subwayaiv2-front-web}:prod"
    
    if docker build -t "$IMAGE_NAME_FULL" -f Dockerfile.offline $BUILD_ARGS .; then
        echo -e "${GREEN}✅ 이미지 빌드 완료: ${IMAGE_NAME_FULL}${NC}"
    else
...
start_services() {
    echo -e "${YELLOW}🚀 운영 환경 서비스 시작 중 (오프라인 모드)...${NC}"
...
show_status() {
    ...
    if [ "$HTTP_CODE" = "200" ] || [ "$HTTP_CODE" = "304" ]; then
        ...
        echo -e "${YELLOW}로그를 확인하세요: ./scripts/offline/run-prod-offline.sh logs${NC}"
    else
...
main() {
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}  운영 환경 오프라인 실행 스크립트${NC}"
    echo -e "${GREEN}========================================${NC}"
    echo ""
    ...
    # 이미지 확인
    IMAGE_NAME_FULL="${IMAGE_NAME:-subwayaiv2-front-web}:prod"
    if ! docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "^${IMAGE_NAME_FULL}$"; then
        echo -e "${YELLOW}[INFO] 이미지가 없으므로 빌드를 시작합니다.${NC}"
        build_image
    fi
    ...
    echo -e "${CYAN}  logs:${NC}   ./scripts/offline/run-prod-offline.sh logs"
    echo -e "${CYAN}  stop:${NC}   ./scripts/offline/run-prod-offline.sh stop"
    echo -e "${CYAN}  down:${NC}   ./scripts/offline/run-prod-offline.sh down"
    echo -e "${CYAN}  status:${NC} ./scripts/offline/run-prod-offline.sh status"
    echo -e "${CYAN}  build:${NC}  ./scripts/offline/run-prod-offline.sh build"
}
