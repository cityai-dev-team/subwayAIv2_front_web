...
export default function MonthlyStatistics() {
  ...
  const fetchData = useCallback(async (e?: React.MouseEvent) => {
    ...
    setLoading(true);
    try {
      // 월별 통계이지만 백엔드는 start_date와 end_date를 기대함
      // 해당 월의 첫날과 마지막날로 변환
      ...
      }>('/report/getMonthlyStatistics', { params });
  ...
  const getChartData = (type: string) => {
    ...
    if (isDistribution) {
      ...
      if (selectedCCTV !== 'all') {
        ...
        if (selectedCCTVData) {
          // 테이블 데이터에서 해당 CCTV의 데이터만 필터링하여 월별 집계
          ...
          const cctvChartData = labels.map(label => {
            const chartItem = chartData.find(d => {
              const timeStr = d.time as string;
              const month = timeStr.split(' ')[0] || timeStr;
              return month === label;
            });
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `월별_혼잡_현황_${dateRange[0].format('YYYY-MM')}_${dateRange[1].format('YYYY-MM')}.csv`);
  ...
  const columns = [
    {
      title: <div style={{ textAlign: 'center' }}>월</div>,
      dataIndex: 'time',
      key: 'time',
      width: 160,
      fixed: 'left' as const,
  ...
  return (
    <div style={{ padding: '24px', background: '#f0f2f5', minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <Title level={2} style={{ margin: 0 }}>월별 혼잡 현황</Title>
        <Text type="secondary">
          월별 혼잡도 통계
        </Text>
      </div>
      ...
      <Card 
        ...
        extra={
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Text type="secondary" style={{ fontSize: '12px' }}>* 15초간 데이터의 1개월 통계</Text>
